...
export default {
  ...
  methods: {
    ...
    sendTicket(){
        ...
        http.post("http://localhost:8181/private/createTicket", translatedTicket ,{ headers : {
            "Authorization" : this.$store.state.token
        }})
        .then(resp => {this.$router.push('/tickets'); resp.data = null; })
        .catch(err => this.$toast.error(err));
    },
    getUserId(name){
      if(name.trim() === ""){
      ...
      this.$toast.warning("That user is not found! Ticket created on your name.")
